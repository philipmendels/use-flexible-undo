### Initial history - Readme & Code

You can use the **initialHistory** prop to initialize the history state with static data. If you create this data manually then make sure that you have an initial action (e.g. with type 'start'). This initial action cannot be undone but can be clicked in the history UI to undo the first user action.
